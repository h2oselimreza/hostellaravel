...
class ExpenseRepository
{
    ...
    // }
        ...
        public function getExpenseSummary(array $arr)
        {
            return DB::table('expense_summary')
                ...
                ->when(!empty($arr['expenseType']), function ($query) use ($arr) {
                    $query->where('expense_summary.expense_type', $arr['expenseType']);
                })
                ->when(!empty($arr['expenseNo']), function ($query) use ($arr) {
                    $query->where('expense_summary.expense_no', $arr['expenseNo']);
                })
                ->orderBy('expense_summary.created_dt_tm', 'desc')
                ->get();
        }

    public function getExpenseDetails(array $arr)
    {
        return DB::table('expense_detail')
            ->select(
                'expense_detail.*',
    ...
    public function getExpenseFiles(array $arr)
    {
        return DB::table('expense_files')
            ->where('expense_no', $arr['expenseNo'])
            ->get();

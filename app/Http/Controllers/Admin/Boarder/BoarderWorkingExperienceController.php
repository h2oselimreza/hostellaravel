...
use App\Models\Admin\BoarderWorkingExperience;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class BoarderWorkingExperienceController extends Controller
{
    public function edit($boarderId){
        $data = Boarder::where('boarder_id',$boarderId)->first();
        $workingDetails = $this->getEmpWorkingDetails($boarderId);
        return view("admin.boarder.add-boarder.working-experience.edit",compact("data",'workingDetails'));
    }
    ...
    public function update(Request $request, $boarderId)
    {
        ...
        DB::transaction(function () use ($request, $boarderId, $workingExpCount, $deleteRows) {
        
            // 1️⃣ Delete removed rows
            if (!empty($deleteRows)) {
                BoarderWorkingExperience::whereIn('id', $deleteRows)->delete();
            }
            //dd($request->all());
            ...
            for ($i = 1; $i <= $workingExpCount; $i++) {
                ...
                if ($request->filled("hiddenWorkingDiv{$i}")) {
                    $workingExp = BoarderWorkingExperience::find($request->input("hiddenWorkingDiv{$i}"));
                } else {
                    $workingExp = new BoarderWorkingExperience();
                    $workingExp->boarder_id = $boarderId;
                }
                //dd($i, $request->input("institutionName{$i}"));
    ...
    private function getEmpWorkingDetails($boarderId = null, $boarderIdArray = [])
    {
        $query = BoarderWorkingExperience::query();

         if ($boarderId) {
            $query->where('boarder_id', $boarderId);
         } elseif (!empty($boarderIdArray)) {
            $query->whereIn('boarder_id', $boarderIdArray);
         }
